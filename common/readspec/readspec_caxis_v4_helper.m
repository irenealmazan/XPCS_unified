...
% version 20160213 
% version 20160309 changing so that it figures out the order geomot
%
...
function [OM,GeoMot] = makeOM(G1,Olabels)
	...
	% This is assumed order saved in OM (see lines G1) and usually are the same as output in 'pa' command
	...
	OM.cparam 	= G1(1:6)';
	OM.h0		= G1(13:15)';  %%<<< much match number and order of geo labels
	OM.h1		= G1(16:18)';
...
% Note extra line below added so that this chan2col will only give the 
%	first instance of a match, and not all (need this to ignore header issues)
...
for ii = 1:length(channelchoose(:,1))
	THE_MATCH 	= find( strcmp(strtrim(channelchoose(ii,:)),channames_as_cellarray));
		if ~isempty(THE_MATCH)
			THE_MATCH = THE_MATCH(1);  %%% here is where it kicks out multiples
