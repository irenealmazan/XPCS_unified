...
for jj=1:NPTS;
	...
	warning('on','MATLAB:imagesci:tifftagsread:zeroComponentCount');  % turns back on
	
end	

if nargin ==3 & ~isempty(badpixfile)
	% badpixfile can be filename or can be the matrix itself
	[ImageStack,BadPixDocu] = apply_badpixreplace(ImageStack,badpixfile);
	imdocu = BadPixDocu;
end
