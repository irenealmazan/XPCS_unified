...
% within our group (SRS) our papers using the calc_hkl have been on hexagonal, and cubic rlu
	astar1	=2*pi/(cparam(1)*sind(cparam(5))*sind(cparam(6)));
	bstar1	=2*pi/(cparam(2)*sind(cparam(6))*sind(cparam(4)));
	cstar1	=2*pi/(cparam(3)*sind(cparam(4))*sind(cparam(5)));
	...
	DEV = [astar-astar1 bstar-bstar1 cstar-cstar1];
	disp(['Dev of old star and new ',num2str(DEV)]);
