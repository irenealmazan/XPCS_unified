...
%	INFOinterest = ['Sam_Z  stemp  TABLEVERT  Delta  Chi  Eta  s6vgap  s6hgap'];     %!!! put 2 spaces between each!!!!
	STR.imname='pil_';  % caxis non-unique naming scheme for images (sigh) 
else
...
% make some of the images to be ready for other work, some is legacy from pixirad windowing
		...
		elseif length(BKG)>1
			...
			%IIimbkg=0;
		    IIimbkg =  image_est_bkg(IInorm(:,:,[2:lastframes(1)]),'Y',BKGshoulders);  % pre 2018_0315_1
		    else
		    %IIimbkg=0;
...
%	 			if ~isempty(AXISdet); set(gca,'Ylim',AXISdet(3:4));end
	...
	plot_adjust(gca,[0 1]);
		...
		HS.CData(:,widelastframes)=zeros(size(HS.CData(:,widelastframes))).*100+max(get(gca,'zlim'));  %colors valves in surface
	 	HYline = makeyline(Xsteps(lastframes),'b',gca);  % and makes a straight line
		end
