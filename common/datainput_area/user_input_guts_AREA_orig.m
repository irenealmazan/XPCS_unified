...
[NameMatrix,sdata]	= make_imnames_2018_03(HDFfilename,SCNs,STR);
...
%		older ones, not using the ones that attempt to scale it back to consistent (to compare 'absolute' between changes)
...
%	valve_v	= sdata.DATA(:,chan2col(sdata.collabels,'valve_v'));
	...
	lastframes = find(diff(valve)>=diffvalve);  % this is in spec points, when using to index matlab point +1
...
%		HS = pcolor(XCOLpts,YROWpts,log10(fixlogimage((IInormb(:,:,SINGLE+ImageJ)))));
		HS = imagesc(XCOLpts,YROWpts,log10(fixlogimage((IInormb(:,:,SINGLE+ImageJ)))));
		shading flat
		HSurf.IInormb = HS;
...
for ii=N_ROIS_SUM; %1%:length(ROIS(:,1));
  iii=iii+1;
		...
		% for CDATA lines of valves, need a bit more oomph, lastframes is column vector if more than one
		% need the extra +1 to convert spec point numbers to matlab indices
  		if ~isempty(lastframes)
%			widelastframes = cat(2,lastframes-1,lastframes,lastframes+1);
			widelastframes = cat(2,lastframes-1+1,lastframes,lastframes+1+1);
			widelastframes = reshape(widelastframes',length(widelastframes(:)),1);
		end
...
%	 HS = surf(Xsteps,SoX.ndx{ii},log10(fixlogimage(SoX.images{ii})),log10(fixlogimage(SoX.images{ii})));
	 HS = imagesc(Xsteps,SoX.ndx{ii},log10(fixlogimage(SoX.images{ii})));
	 end
...
%	 			if ~isempty(AXISdet); set(gca,'Ylim',AXISdet(3:4));end
	...
	plot_adjust(gca,[0 1]);
		...
		if ~isempty(lastframes)
		HS.CData(:,widelastframes)=zeros(size(HS.CData(:,widelastframes))).*100+max(get(gca,'zlim'));  %colors valves in surface
...
%	 HL = plot(Xsteps,log10(sumROISnormed)); YLABEL = 'log10[Inorm(summed over ROI)/ROIsize]';
	 ...
	 plot_adjust(gca,0); %
% mocvd
	 makeyline(Xsteps(lastframes+1),'b',gca);   % want colored lines on this plot, not white	 
	 for ii=1:length(ROIS(:,1));
		set(HL(ii),'Color',COLORORDER(ii,:),'LineWidth',2);
	 end
...
for ii = 1:length(POINTS(:,1))
	...
	if ~LOGFLAG
		...
		HS = imagesc(XCOLpts,YROWpts,log10(fixlogimage(sum(IInormb(:,:,Ni),3)./(NP))));
		colormap(COLORMAP)	
			if ~ASPECTflag
				axis equal;
			else
