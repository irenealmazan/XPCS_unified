...
% Modified by Brian Stephenson 29-Aug-96
%          uses semilogy plot
%
% References:
...
plotcmd='semilogy(x(:,1),y,''+'',x(:,1),f); shg';
%if (sscanf(version,'%f') >= 4),
vernum= sscanf(version,'%f');
if vernum(1) >= 4,
  global verbose
%  plotcmd='plot(x(:,1),y,''+'',x(:,1),f); figure(gcf)';
  plotcmd='semilogy(x(:,1),y,''+'',x(:,1),f); figure(gcf)';
end;

if(exist('verbose')~=1), %If verbose undefined, givem everything
	verbose(1)=1;    %This will tell them the results
	verbose(2)=1;    %This will replot each loop
end;
