...
%
%  use _CW for zaxis, sevchex: if sigma = |sigma| then, tau is -PHImax-90
%		(tau definition OK for zaxis, and sevchex) 
%  use other function _CCW for caxis, sevchub: if sigma = |sigma| then, tau is +PHImax-90
%		
%
...
sigmatau.docu		= strvcat(date,...
	...
	['tau calculated from -90 - PHI_max (CW rotations) sigma is tilt ']); 
...
HHxaxis = line([-sigmatau.tau -sigmatau.tau],[-AMPLITUDE AMPLITUDE]+ALPHA-rho);
	...
	TITLEstr = strvcat(['Tau  = ',num2str(sigmatau.tau),' (=-90-Max (CW), Max at ',num2str(PHIMAX),' )'],...
		[' TILT (sigma) = ',num2str(AMPLITUDE)],...
